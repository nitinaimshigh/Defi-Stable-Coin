...
// Handler is going to narrow down the ways in which we call functions
...
contract Handler is StdInvariant, Test {
    ...
    uint256 MAX_DEPOSIT_SIZE = type(uint96).max; // allows us to get max96 value;
    ...
    constructor(DSCEngine _dscEngine, DecentralisedStableCoin _dsc) {
        dsce = _dscEngine;
        dsc = _dsc;
        address[] memory collateralTokens = dsce._getCollateralTokens();
